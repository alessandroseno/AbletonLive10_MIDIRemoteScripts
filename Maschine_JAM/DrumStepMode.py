# Embedded file name: C:\ProgramData\Ableton\Live 10 Suite\Resources\MIDI Remote Scripts\Maschine_JAM\DrumStepMode.py
...
class VelocityPad:
    ...
    def assign_button(self, button):
        if button:
            ...
            self.__button.add_value_listener(self.handle_selection)
        else:
            self.__button.remove_value_listener(self.handle_selection)
            self.__button = None
        return

    def handle_selection(self, value):
        if value:
            self.__callback(self.index)

    def update_button(self, selected):
        if selected:
            self.__button.send_color_direct(38)
        else:
            self.__button.send_color_direct(36)
...
class DrumStepPad:
    ...
    def __init__(self, index, callback):
        ...
        self.__button = None
        self.__callback = callback
        self.__color = colorOnOff(CI_WHITE)
        self.__note = None
        return
    ...
    def set_color(self, color):
        self.__color = color
        self.__button.send_color_direct(color)

    def brighten(self):
        self.__button.send_color_direct(123)

    def restore(self):
        self.__button.send_color_direct(self.__color)
...
class DrumStepMode(MaschineMode):
    ...
    __selected_velocity_index = 15
    __pad_selection_only = False
    __pad_select_direct = False
    ...
    def __init__(self, button_index, monochrome=False, *a, **k):
        ...
        self.__vel_pads = tuple((VelocityPad(index, self.vel_changed) for index in range(16)))
        self.__step_pads = tuple((DrumStepPad(index, self.step_selected_changed) for index in range(32)))
    ...
    def page_index(self):
        ...
        return (
         int(self._pos_scroll / seg_len), max(1, int(self._clip.length / seg_len)))
    ...
    def set_run_light(self, run_index):
        if run_index != self._current_runindex:
            if self._current_runindex != -1:
                self.__step_pads[self._current_runindex].restore()
            self._current_runindex = run_index
            if self._current_runindex != -1:
                self.__step_pads[run_index].brighten()

    def vel_changed(self, index):
        if index != self.__selected_velocity_index:
            self.__vel_pads[self.__selected_velocity_index].update_button(False)
            self.__selected_velocity_index = index
            self.__vel_pads[self.__selected_velocity_index].update_button(True)
        ...
        if self._modifier_componenet.is_shift_down():
            ...
            if self._istriplet:
                ...
                if colx >= 6:
                    return
                index = rowx * 6 + colx
            step = self.__step_pads[step_index]
            note = self.device.view.selected_drum_pad.note
            position = self._pos_scroll + self._quantize * index
            if step.note:
                self._clip.remove_notes(position, note, self._quantize, 1)
            else:
                self._clip.set_notes(tuple([
                 (note, position, self._quantize,
                  VEL_MAP[self.__selected_velocity_index], 0)]))
    ...
    def _assign_notes(self):
        if not self._clip:
            return
        note = self.device.view.selected_drum_pad.note
        notes = self._clip.get_notes(0.0, note, self._clip.length, 1)
        for step in self.__step_pads:
            step.set_note(None)

        for note in notes:
            ci = (note[1] - self._pos_scroll) / self._quantize
            if ci < 0:
                colindex = -1
            else:
            ...
            if self._istriplet:
                if colindex in range(24):
                    self.__step_pads[wrap_triplet_index(colindex)].set_note(note)
            elif colindex in range(32):
                self.__step_pads[colindex].set_note(note)
    ...
    def handle_shift(self, shift_value):
        self.__pad_selection_only = shift_value
    ...
    def notify_edit_action(self, val, action_type, modifier):
        ...
        if action_type == CLEAR_ACTION and self._clip:
            if modifier:
                self._clip.remove_notes(0.0, 0, self._clip.length, 128)
            else:
                self._clip.remove_notes(0.0, note, self._clip.length, 1)
        else:
            if action_type == DUPLICATE_ACTION and self._clip and modifier:
    ...
    def clear_clip(self, option=False):
        if not self._clip:
            return
        if option:
            self._clip.remove_notes(self._pos_scroll, 0, self._quantize * 8, 128)
        else:
            self._clip.remove_notes(0.0, 0, self._clip.length, 128)
    ...
    def hande_drum_pad_selection(self, index, value):
        if value and self.device:
            ...
            if modmode == MASK_CLEAR:
                self._clip.remove_notes(0, self.device.view.selected_drum_pad.note, self._clip.length, 1)

    def reassign_drum_pads(self):
        for pad in self._pads:
            pad.set_notemode(not self.__pad_selection_only)
            if not self.__pad_selection_only:
                button = pad.button
            ...
            if button:
                if row > 3:
                    if column > 3:
                        ...
                        self.__vel_pads[vel_index].assign_button(button)
                        self.__vel_pads[vel_index].update_button(vel_index == self.__selected_velocity_index)
                else:
                    step_index = row * 8 + column
                    self.__step_pads[step_index].assign_button(button)
                    self.__step_pads[step_index].set_color(None)
    ...
    def exit(self):
        if self._active and self.canonical_parent and self.canonical_parent._bmatrix:
            matrix = self.canonical_parent._bmatrix
            matrix.prepare_update()
            for button, (column, row) in matrix.iterbuttons():
                if button:
                    ...
                    if row < 4:
                        step_index = row * 8 + column
                        self.__step_pads[step_index].assign_button(None)
                    elif column < 4:
                        vel_index = (7 - row) * 4 + column
                        self.__vel_pads[vel_index].assign_button(None)
    ...
    def _handle_playing(self):
        if self._run_light_handler and not self._clip.is_playing:
            self._run_light_handler.set_run_light(-1)

    @subject_slot('loop_end')
    def _handle_loop_end_changed(self):
        self._page_handler.update_buttons()
        if self._run_light_handler and self._length_listener:
