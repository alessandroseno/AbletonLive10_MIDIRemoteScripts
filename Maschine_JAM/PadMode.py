# Embedded file name: C:\ProgramData\Ableton\Live 10 Suite\Resources\MIDI Remote Scripts\Maschine_JAM\PadMode.py
...
class PadMode(MaschineMode):
    __modifier_component = None
    _editmode = None
    _focus_track = None
    __layout = LAYOUT_ISO_4_INKEY
    ...
    def get_color(self, value, column, row):
        ...
        if button is not None:
            ...
            if self.__layout == LAYOUT_ISO_3_CHROM or self.__layout == LAYOUT_ISO_4_CHROM:
                full_color = color[1]
    ...
    def inc_layout(self, inc):
        new_value = self.__layout + inc
        if new_value < 0:
            self.__layout = LAYOUT_ISO_3_CHROM
        else:
            if new_value > LAYOUT_ISO_3_CHROM:
                self.__layout = LAYOUT_SPREAD
            else:
                self.__layout = new_value
        self.canonical_parent.show_message(' Pad Layout: ' + LAYOUT_NAME[self.__layout] + ' / ' + self._current_scale_name(SCALES[self.current_scale_index]))
    ...
    def set_modifier_component(self, component):
        self.__modifier_component = component
    ...
    def handle_button(self, value, button):
        if value != 0:
            ...
            if row == 0:
                self.__modifier_component.handle_edit_action(col)

    @subject_slot('selected_track')
    def on_track_changed(self):
        self._focus_track = self.__determine_focus_track()
        self.refresh()
