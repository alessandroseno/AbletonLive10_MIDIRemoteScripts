# Embedded file name: C:\ProgramData\Ableton\Live 10 Suite\Resources\MIDI Remote Scripts\Maschine_JAM\StepMode.py
...
from MidiMap import COLOR_BLACK, quantize_settings, qauntize_is_triplet, quantize_base, quantize_clip_live, quantize_string, STEP_MODE, toHSB, BASE_NOTE_FIX_COLOR, from_midi_note
...
class GridElement:
    ...
    def restore(self):
        self._button.send_color_direct(self.__restore_color)

    def note_finalize(self, force=False):
        if not self.__tmp and self._notes:
            self._notes = None
            self._button.send_value(COLOR_BLACK)
        else:
            if self.__tmp and not self._notes:
                self._notes = self.__tmp
                self._button.send_value(COLOR_BLACK)
            else:
                if self.__tmp and self._notes:
                    self._notes = self.__tmp
    ...
    def color(self):
        if self._notes is None:
            if self.is_base_note:
                self.__restore_color = self._note_handler.bs_color
                return self._note_handler.bs_color
            self.__restore_color = COLOR_BLACK
            return
        if self.is_base_note:
            self.__restore_color = self._note_handler.on_color
            return self._note_handler.on_color
        self.__restore_color = self._note_handler.on_color
        return self._note_handler.on_color

    def __str__(self):
        return 'GRID(' + str(self._row) + ',' + str(self._col) + ') Notes = ' + str(self.__tmp)
...
class StepMode(MaschineMode):
    ...
    def select_note(self, grid):
        ...
        if self._modifier_componenet.is_shift_down():
            if row == 0:
                self._modifier_componenet.handle_edit_action(col, (self._padmode.get_base_note(), self._padmode.get_scale))
        else:
            ls = []
            noteval = self._padmode.get_scale.grid_row_to_note(row)
            if grid.has_notes():
                self._clip.remove_notes(self._pos_scroll + self._quantize * col, noteval, self._quantize, 1)
            else:
                if self._mono_mode:
                    self._clip.remove_notes(self._pos_scroll + self._quantize * col, 0, self._quantize, 128)
                ls.append((noteval, self._pos_scroll + self._quantize * col, self._quantize * self._note_length, self._in_velocity, 0))
                self._clip.set_notes(tuple(ls))
    ...
    def note_to_grid(self, note, scale, scale_base_note):
        ...
        if rowindex in range(8) and colindex in range(self._quantize_grid_max):
    ...
    def level_change(self, col, value):
        if self._step_values[col] is not None:
            if len(self._steps_notes[col]) > 0:
                ...
                self._clip.set_notes(tuple(update_notes))
        return
    ...
    def _handle_play_head(self):
        ...
        if relpos < 0:
            ...
            colindex = int(relpos)
        if colindex in xrange(self._quantize_grid_max):
    ...
    def set_run_light(self, run_index):
        if run_index != self.__current_run_index:
            if self.__current_run_index != -1:
                for rowindex in xrange(8):
                    self._grid[self.__current_run_index][rowindex].restore()

            self.__current_run_index = run_index
            if self.__current_run_index != -1:
                for rowindex in xrange(8):
    ...
    def repeat_section(self, option=False):
        if not self._clip:
            return
        notes = self._clip.get_notes(self._pos_scroll, 0, self._quantize * 8, 127)
        ...
        if sections > 1:
            ...
            for sec_index in range(sections):
                ...
                for note in notes:
                    ...
                    ls.append((noteval, copypos + basepos, note[2], note[3], note[4]))
