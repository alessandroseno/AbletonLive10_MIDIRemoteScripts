...
MIDI_SYSEX_TYPE = 3
MIDI_INVALID_TYPE = 4
MIDI_MSG_TYPES = (MIDI_NOTE_TYPE, MIDI_CC_TYPE,
 MIDI_PB_TYPE, MIDI_SYSEX_TYPE,
 MIDI_INVALID_TYPE)
...
class InputControlElement(NotifyingControlElement):
        ...
        if self._mapping_feedback_delay != 0:
            if self._msg_type != MIDI_PB_TYPE:
                ...
                for value in xrange(16384):
                    value_pairs.append((value >> 7 & 127,
                     value & 127))
    ...
    def send_value(self, value, force=False, channel=None):
        ...
        if force or self._force_next_send:
            ...
            if self.send_depends_on_forwarding and not self._is_being_forwarded or self._send_delayed_messages_task.is_running:
