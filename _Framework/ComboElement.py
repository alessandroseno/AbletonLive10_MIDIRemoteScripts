...
class ComboElement(WrapperElement):
    ...
    def get_control_element_priority(self, element, priority):
        assert element == self._wrapped_control and priority is None or 1 - priority + int(priority) > self.priority_increment, 'Attempting to increase the priority over a whole unit. ' + 'Make sure the combo element is not inside another combo element'
        if priority is None:
            priority = DEFAULT_PRIORITY if 1 else priority
