...
def parameter_to_bool(parameter):
    ...
    if isinstance(parameter, (int, type(False))):
        ...
        if unicode(parameter) in (u'True', u'False'):
            bool_value = unicode(parameter) == 'True'
    return bool_value
...
class MxDCore(object):
    ...
    def update_device_context(self, device_id, object_id):
        ...
        if object_id not in self.device_contexts[device_id].keys():
            self.device_contexts[device_id][object_id] = {PATH_KEY: [], ID_KEY: 0, TYPE_KEY: None, PROP_KEY: '', PROP_LISTENER_KEY: (None, None, None), 
               PATH_LISTENER_KEY: {}, OPEN_OPERATIONS_KEY: {}, LAST_SENT_ID_KEY: None}
        return
    ...
    def path_goto(self, device_id, object_id, parameters):
        ...
        for msg_type, value in (('path_curr_path', result_path),
         (
    ...
    def path_get_id(self, device_id, object_id, parameters):
        ...
        for msg_type, value in (('path_orig_id', result_id),
         (
          'path_curr_id', result_id)):
            self.manager.send_message(device_id, object_id, msg_type, value)
        ...
        if len(current_path) == 0:
            ...
            if current_object != None:
                ...
                if is_object_iterable(current_object):
                    result = '%d list elements, no %s' % (
                     len(current_object), looking_for)
                else:
                ...
                if isinstance(prop, (int, bool)):
                    ...
                    if isinstance(prop, float):
                        ...
                        if isinstance(prop, (str, unicode)):
                            if not isinstance(value, (str, unicode, int, float)):
                                raise LomAttributeError('set: type mismatch')
                            value = unicode(value)
                        else:
    ...
    def obj_call(self, device_id, object_id, parameters):
        ...
        if current_object != None:
            try:
                ...
                self._raise(device_id, object_id, "%s: '%s'" % (unicode(e), parameters))
    ...
    def _create_notes_output(self, notes):
        ...
        result = 'notes %d\n%sdone' % (
         len(notes),
         concatenate_strings(map(note_format, notes), string_format='%s%s'))
        return result
    ...
    def _observer_install_listener(self, device_id, object_id):
        assert self.device_contexts[device_id][object_id][PROP_LISTENER_KEY] == (None,
                                                                                 None,
                                                                                 None)
        current_object = self._get_current_lom_object(device_id, object_id)
        property_name = self._get_current_property(device_id, object_id)
            ...
            if current_object != None and property_name != '':
                ...
                if hasattr(current_object, transl_prop_name + '_has_listener'):
                    ...
                    object_context[PROP_LISTENER_KEY] = (
                     listener_callback, current_object, property_name)
                    listener_callback()
    ...
    def _observer_id_callback(self, device_id, object_id):
        assert self._get_current_property(device_id, object_id) == 'id'
        object_context = self.device_contexts[device_id][object_id]
        current_object = self._get_current_lom_object(device_id, object_id)
