...
PARAMETERS_LOOPED = (
 'Loop position', 'Loop length', 'Start offset')
    ...
    def create_mode_path(initial_mode_map):
        ...
        def create_mode_path_recursive(mode_map):
            ...
            if selected_mode in children:
                if len(children[selected_mode]['children'].keys()) > 0:
...
class MidiClipControllerComponent(CompoundComponent):
    ...
    def __init__(self, *a, **k):
        ...
        self._visualisation_real_time_data = self.register_component(RealTimeDataComponent(channel_type='visualisation'))
    ...
    def external_regions_of_interest_creator(self, region_of_interest_creator):
        ...
        return {'grid_window': region_of_interest_creator(start_identifier=self.grid_window_focus, getter=lambda : (
                         grid_start_time(),
    ...
    def _focus_grid_window(self):
        if liveobj_valid(self.clip) and self.get_static_view_data()['ShowGridWindow']:
    ...
    def _update_notification_mutes(self):
        for component in chain(self._sequencers, self._instruments):
            if hasattr(component, 'show_notifications'):
