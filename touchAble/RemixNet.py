...
class OSCClient:
    ...
    def __init__(self, touchAble, udpClient=None, address=None, msg=None):
        """
        ...
        #self.touchAble.log_message("OSCClient logging from touchable?")
...
class OSCServer:
    ...
    def __init__(self, touchAble = None, dst=None, dstPort=None, src=None, srcPort=None):
        ...
        self.udpServer = UDPServer(self, src, srcPort)
        self.udpClient = UDPClient(dst, dstPort)
        self.udpClient.open()
        
        self.oscClient = OSCClient(self.touchAble, self.udpClient,None, None)
...
class UDPServer:
    ...
    def bind(self):
        ...
        while True:
            try:
                self.UDPSock.bind((self.src,self.srcPort))
            except:
                self.srcPort += 1
            
            else:
                self.UDPSock.setblocking(0)
                self.oscServer.sendOSC("/script/port_changed", self.srcPort)
                break
