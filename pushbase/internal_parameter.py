...
class InternalParameterBase(EventObject):
    ...
    def state(self, new_state):
        assert new_state in (
         DeviceParameter.ParameterState.enabled,
         DeviceParameter.ParameterState.irrelevant,
         DeviceParameter.ParameterState.disabled)
...
class IntegerParameter(InternalParameter):
    ...
    def _set_integer_value(self, new_value):
        assert self._min_value <= new_value <= self._max_value, ('Invalid value {}').format(new_value)
