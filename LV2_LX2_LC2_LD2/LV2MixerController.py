...
class LV2MixerController(FaderfoxMixerController):
    ...
    def reset_status_cache(self):
        self.status_cache = {'mute': [ -1 for i in range(0, 12) ], 'solo': [ -1 for i in range(0, 12) ], 'arm': [ -1 for i in range(0, 12) ], 'current_monitoring_state': [ -1 for i in range(0, 12) ], 'master': {'solo': -1, 'mute': -1, 
                      'arm': -1}, 
           'master_note': {'solo': MASTER_SOLO_NOTE, 'mute': MASTER_MUTE_NOTE, 
                           'arm': MASTER_ARM_NOTE}}
    ...
    def send_track_status_midi(self, status, note):
        if status:
            self.parent.send_midi((TRACK_CHANNEL_SETUP2 + NOTEON_STATUS,
             note, STATUS_ON))
        else:
            self.parent.send_midi((TRACK_CHANNEL_SETUP2 + NOTEOFF_STATUS,
             note, STATUS_OFF2))
