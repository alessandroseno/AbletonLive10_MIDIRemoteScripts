...
class LC2ChannelStripComponent(ChannelStripComponent):
    ...
    def _on_routing_changed(self):
        if self._track_id < 8:
            ...
            if self._track is not None:
                objs = [
                 self._track.input_routings, self._track.current_input_routing,
                 self._track.input_sub_routings, self._track.current_input_sub_routing,
                 self._track.output_routings, self._track.current_output_routing,
                 self._track.output_sub_routings, self._track.current_output_sub_routing]
                for i in range(4):
    ...
    def _send_routings(self):
        if self._track_id < 8:
            if self._track is not None:
                for id, type in enumerate([self._track.input_routings, self._track.input_sub_routings, self._track.output_routings, self._track.output_sub_routings]):
        ...
        if hasattr(self, '_get_offset'):
            ...
            if self._track_id < offsets[2]:
                ...
                if self._track in self.song().tracks:
                    ...
                    if self._track in self.song().return_tracks:
    ...
    def set_routing(self, type, val):
        ...
        if self._track is not None:
            types = [
             'current_input_routing', 'current_input_sub_routing', 'current_output_routing', 'current_output_sub_routing']
            vals = [self._track.input_routings, self._track.input_sub_routings, self._track.output_routings, self._track.output_sub_routings]
            if type < len(types):
