...
# Compiled at: 2014-07-11 15:15:12
...
class LC2Modulator(DeviceComponent):
    ...
    def handle_sysex(self, sysex):
        cmds = [
         self._select_xy, self._set_param, self._select_param, self._select_chain, self._select_chain_device, self._select_parent]
        if sysex[0] < len(cmds):
    ...
    def set_device_select_buttons(self, buttons):
        assert isinstance(tuple, buttons)
