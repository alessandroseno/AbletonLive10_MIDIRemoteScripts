...
class LC2SessionComponent(SessionComponent):
    ...
    def handle_sysex(self, sysex):
        cmds = [
         self._launch_clip, self._launch_scene, self._jump_clip, self._set_rows, self._set_cols, self._track_stop, self._stop_all, self._set_size, self._select_clip, self._snapshot, self._select_send]
        if sysex[0] < len(cmds):
    ...
    def _get_offset(self):
        return (
         self._track_offset, self._scene_offset, self._width, self._height)
    ...
    def _bank_up_value(self, value):
        ...
        if self.is_enabled():
            ...
            if not self._is_scrolling() and (value is not 0 or not button_is_momentary):
                self.set_offsets(self._track_offset, min(len(self.song().scenes) - self.height(), self._scene_offset + self.height()))
                self.send_offsets()
        return
    ...
    def _bank_down_value(self, value):
        ...
        if self.is_enabled():
            ...
            if not self._is_scrolling() and (value is not 0 or not button_is_momentary):
                self.set_offsets(self._track_offset, max(0, self._scene_offset - self.height()))
                self.send_offsets()
        return
