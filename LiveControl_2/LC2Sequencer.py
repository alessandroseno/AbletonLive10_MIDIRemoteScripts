...
# Embedded file name: /Applications/Ableton Live 9 Suite.app/Contents/App-Resources/MIDI Remote Scripts/LiveControl_2_1_31/LC2Sequencer.py
...
class LC2Sequencer(ControlSurfaceComponent):
    ...
    def __init__(self):
        ...
        self._chord_intervals = [
         [
          0, 4, 7], [0, 3, 7], [0, 4, 8], [0, 4, 6], [0, 3, 6], [0, 5, 7]]
        self._quick_chord = 127
        self._durations = [
         0.125, 0.25, 0.5, 0.75, 1, 1.25, 1.5, 2, 4]
        self._selection = [
         0, 0, 0, 0]
        self._last_details = [
         -1, -1, -1, -1, -1]
    ...
    def handle_sysex(self, sysex):
        cmds = [
         self._note_press, self._fader_press, self._select_note, self._mute_note,
         self._scroll_time, self._scroll_note, self._zoom,
         self._set_quick_chord, self._set_selection, self._midi_action, self._set_root_scale,
         self._randomise_fader, self._set_defaults, self._set_fader_fn, self._set_height,
         self._set_fold, self._set_save]
        cmd = sysex[0]
        if cmd < len(cmds):
            if self._clip is not None:
    ...
    def _midi_action(self, sysex):
        acts = [
         self._arpeggiate, self._randomise_steps, self._duplicate, self._transpose, self._double, self._strum, self._flip, self._compress, self._harmonise, self._undo, self._move, self._clear, self._half]
    ...
    def _arpeggiate(self, args):
        ...
        for step in range(steps):
            ...
            new_notes.append([pitch, note[1] + step * interval, interval * gate, note[3], False])
    ...
    def _duplicate(self, args):
        if self._clip is not None:
            new_notes = []
            for note in self._note_cache:
                new_notes.append([note[0], note[1] + self._clip.length, note[2], note[3], note[4]])
                ...
                if self._scale == 0:
                    if self._quick_chord < len(self._chord_intervals):
                        for offset in self._chord_intervals[self._quick_chord]:
                            notes.append([pitch + offset, pos, self._quantisation * self._default_length, self._default_velocity, False])

                else:
                    for i in range(self._quick_chord + 3):
                        ns = self._note_step(pitch)
                        notes.append([self._note(ns + i * 2), pos, self._quantisation * self._default_length, self._default_velocity, False])

            else:
                if self._last_details[1] >= pos and self._last_details[1] < pos2 and self._save_note:
                    notes.append([pitch, self._last_details[1], self._last_details[2], self._last_details[3], self._mutes[pitch]])
                    self._last_details = [-1, -1, -1, -1, -1]
                else:
                    notes.append([pitch, pos, self._quantisation * self._default_length, self._default_velocity, self._mutes[pitch]])
            self._clip.deselect_all_notes()
    ...
    def rem_note(self, pos, pos2, pitch):
        if self._clip is not None:
            ...
            for nt in self._note_cache:
                if nt[0] == pitch and nt[1] >= pos and nt[1] < pos2:
                    self._last_details = nt
                else:
                    new_notes.append(nt)
