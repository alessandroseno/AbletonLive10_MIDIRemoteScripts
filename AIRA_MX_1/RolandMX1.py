...
# Compiled at: 2018-05-12 16:22:09
...
class RolandMX1(OptimizedControlSurface):
    ...
    def _create_controls(self):
        ...
        self._tone_filter_knobs = ButtonMatrixElement(rows=[
         [ make_encoder('Tone_Filter_Encoder_%d' % (col,), identifier) for col, identifier in enumerate(xrange(102, 113))
         ]], name='Tone_Filter_Knobs')
        step_buttons_raw = [
         [ make_button('Step_Button_%d' % (col,), identifier) for col, identifier in enumerate(xrange(20, 31))
         ]]
        ...
        self._select_buttons = ButtonMatrixElement(rows=[
         [ make_button('Select_Button_%d' % (col,), identifier) for col, identifier in enumerate(xrange(60, 71))
         ]], name='Select_Buttons')
        self._bfx_buttons = ButtonMatrixElement(rows=[
         [ make_button('BFX_Button_%d' % (col,), identifier) for col, identifier in enumerate(xrange(80, 91))
         ]], name='BFX_Buttons')
        self._mfx_buttons = ButtonMatrixElement(rows=[
         [ make_button('MFX_Button_%d' % (col,), identifier) for col, identifier in enumerate(xrange(100, 111))
         ]], name='MFX_Buttons')
