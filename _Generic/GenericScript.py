...
class GenericScript(ControlSurface):
    ...
    def _init_mixer_component(self, volume_controls, trackarm_controls, mixer_options, global_channel, volume_map_mode):
        momentary_buttons = mixer_options is not None and 'NOTOGGLE' in mixer_options.keys()
        ...
        if volume_controls != None and trackarm_controls != None:
            ...
            if mixer_options != None:
                if has_specification_for('MASTERVOLUME', mixer_options) and is_valid_midi_identifier(mixer_options['MASTERVOLUME']):
                ...
                for send in xrange(mixer_options.get('NUMSENDS', 0)):
            ...
            for track in xrange(num_strips):
                ...
                if 0 <= track < len(trackarm_controls) and is_valid_midi_identifier(trackarm_controls[track]):
                    layer_specs['arm_button'] = MixerButton(trackarm_controls[track], name=str(track) + '_Arm_Button')
                send_controls_raw = []
                for index, send in enumerate(send_info):
                    if 0 <= track < len(send):
                        ...
                        if is_valid_midi_identifier(cc) and is_valid_midi_channel(channel):
                            send_controls_raw.append(make_mixer_encoder(cc, channel, name='%d_Send_%d_Control' % (track, index)))
