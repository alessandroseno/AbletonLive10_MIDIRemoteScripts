# Embedded file name: C:\ProgramData\Ableton\Live 9 Suite\Resources\MIDI Remote Scripts\Maschine_MK3\EncoderHandler.py
# Compiled at: 2017-09-20 23:19:39
...
class EncoderHandler(CompoundComponent):
    ...
    def assign_parameter(self, parameter, track=None, disable_grid=True, use_metering=False):
        ...
        if track:
            ...
            self._name_changed.subject = self._track
            ...
            self._output_meter_changed.subject = None
            self._output_meter_changed_left.subject = None
            self._name_changed.subject = None
    ...
    def _output_meter_changed(self):
        if self._track and self._use_metering:
    ...
    def _name_changed(self):
        self._encoder_control.update_track_names()
    ...
    def _device_change(self):
        if self._track.has_audio_output:
            ...
            self._output_meter_changed.subject = self._track
            self._output_meter_changed_left.subject = None
        if not self._has_audio and self._track.has_audio_output:
            self._has_audio = True
        else:
            if self._has_audio and not self._track.has_audio_output:
                self._has_audio = False
        return
    ...
    def _handle_touch(self, value):
        ...
        if self._parameter and value != 0:
