# Embedded file name: C:\ProgramData\Ableton\Live 9 Suite\Resources\MIDI Remote Scripts\Maschine_MK3\DrumMode.py
...
from MidiMap import *
from Constants import PAD_MODE
...
class DrumMode(MaschineMode):
    ...
    def __init__(self, monochrome=False, *a, **k):
        super(DrumMode, self).__init__(*a, **k)
        self.track = None
        self.device = None
    ...
    def navigate(self, direction, modifier, alt_modifier=False):
        if self.device and self.device.view:
            self.device.view.drum_pads_scroll_position = max(0, min(28, self.device.view.drum_pads_scroll_position + direction))
