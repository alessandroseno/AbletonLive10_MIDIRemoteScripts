...
def make_all_encoders(name_prefix='', make_encoder=make_launch_control_encoder):
    return (
     [ make_encoder(41 + index, name_prefix + '_Bottom_Encoder_' + str(index)) for index in xrange(8)
     ],
     [ make_encoder(21 + index, name_prefix + '_Top_Encoder_' + str(index)) for index in xrange(8)
     ])
...
class LaunchControl(ControlSurface):
    ...
    def _init_modes(self):
        ...
        self._modes.add_mode('device', [
         self._device, self._device_navigation,
         self._view_control])
